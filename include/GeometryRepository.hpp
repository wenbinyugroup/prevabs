...
/**
 * Owns named dcel::PDCELVertex points and Baseline curves with O(1) name lookups.
 *
...
public:
  ...
  void addVertex(dcel::PDCELVertex* v);
  void addBaseline(Baseline* l);

  // -----------------------------------------------------------------
  // Getters

  dcel::PDCELVertex* getPointByName(const std::string& name) const;
  ...
  std::vector<dcel::PDCELVertex*>& vertices()  { return _vertices; }
  std::vector<Baseline*>&    baselines() { return _baselines; }

private:
  std::vector<dcel::PDCELVertex*> _vertices;
  std::vector<Baseline*>    _baselines;

  std::unordered_map<std::string, dcel::PDCELVertex*> _vertex_map;
