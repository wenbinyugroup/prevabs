...
private:
  ...
  int _writeMSH2(const std::string &name, double version = 2.2,
                 ...
                 bool multipleView = false, bool renumberVertices = true);
...
// Required for python bindings on Windows
  ...
  GEdge *addCircleArcCenter(double x, double y, double z, GVertex *start,
                            GVertex *end);
  GEdge *addCircleArcCenter(GVertex *start, GVertex *center, GVertex *end);
  GEdge *addCircleArc3Points(double x, double y, double z, GVertex *start,
                             GVertex *end);
  GEdge *addBezier(GVertex *start, GVertex *end,
                   std::vector<std::vector<double>> points);
  GEdge *addBSpline(GVertex *start, GVertex *end,
                    std::vector<std::vector<double>> points);
  ...
  std::vector<GEntity *> extrudeBoundaryLayer(GEntity *e, int nbLayers,
                                              double hLayers, int dir = 1,
                                              int view = -1);
  ...
  GFace *addFace(std::vector<GEdge *> edges,
                 std::vector<std::vector<double>> points);
  ...
  GFace *addCompoundFace(std::vector<GFace *> faces, int type, int split,
                         int num = -1);
  ...
  GEntity *addCylinder(std::vector<double> p1, std::vector<double> p2,
                       double radius);
  GEntity *addTorus(std::vector<double> p1, std::vector<double> p2,
  ...
  GEntity *addCone(std::vector<double> p1, std::vector<double> p2,
                   double radius1, double radius2);
  ...
  void setPeriodicPairOfFaces(int numFaceMaster,
                              std::vector<int> EdgeListMaster, int numFaceSlave,
