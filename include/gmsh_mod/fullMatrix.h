...
class fullVector
{
  ...
  */
  inline void set(int r, scalar v){
    ...
    //              r, _r);
        printf("[FATAL] invalid index to access fullVector : %i (size = %i)", r, _r);
        // std::cout << "[FATAL]" << "invalid index to access fullVector : " << r << " (size = " << _r << ")" << std::endl;
    #endif
    (*this)(r) = v;
  }
...
class fullMatrix
{
 ...
 public:
  ...
  inline scalar get(int r, int c) const
  {
    ...
    //              r, c, _r, _c);
        printf("[FATAL] invalid index to access fullMatrix : %i %i (size = %i %i)", r, c, _r, _c);
    #endif
    return (*this)(r, c);
  }
  ...
  void operator += (const fullMatrix<scalar> &other)
  {
    ...
    //   Msg::Error("sum matrices of different sizes\n");
        printf("[ERROR] sum matrices of different sizes\n");
    for(int i = 0; i < _r * _c; ++i) _data[i] += other._data[i];
  }
  inline scalar operator () (int i, int j) const
  {
    ...
    //   Msg::Fatal("invalid index to access fullMatrix : %i %i (size = %i %i)",
    //              i, j, _r, _c);
        printf("[FATAL] invalid index to access fullMatrix : %i %i (size = %i %i)", i, j, _r, _c);
    #endif
    return _data[i + _r * j];
  }
  ...
  void copy(const fullMatrix<scalar> &a)
  {
    ...
    //   Msg::Fatal("fullMatrix::copy operation is prohibited for proxies, use setAll instead");
        printf("[FATAL] fullMatrix::copy operation is prohibited for proxies, use setAll instead");
    if (_r != a._r || _c != a._c) {
      if(_data && _own_data)
        delete [] _data;
      _r = a._r;
...
#if !defined(HAVE_BLAS)
  {
    ...
    //   Msg::Fatal("fullMatrix size does not match");
        printf("[FATAL] fullMatrix size does not match");
  ...
  inline void transposeInPlace()
  {
    ...
    //   Msg::Error("Not a square matrix (size1: %d, size2: %d)", size1(), size2());
        printf("[ERROR] Not a square matrix (size1: %d, size2: %d)", size1(), size2());
    }
    scalar t;
    for(int i = 0; i < size1(); i++)
      for(int j = 0; j < i; j++) {
...
#if !defined(HAVE_LAPACK)
  {
    ...
    printf("[ERROR] LU factorization and substitution requires LAPACK");
    return false;
  }
#endif
  ;
 bool luFactor(fullVector<int> &ipiv)
#if !defined(HAVE_LAPACK)
  {
    ...
    printf("[ERROR] LU factorization requires LAPACK");
    return false;
  }
#endif
  ;
 bool luSubstitute(const fullVector<scalar> &rhs, fullVector<int> &ipiv, fullVector<scalar> &result)
#if !defined(HAVE_LAPACK)
  {
    ...
    printf("[ERROR] LU substitution requires LAPACK");
    return false;
  }
#endif
  ;
...
#if !defined(HAVE_LAPACK)
  {
    ...
    printf("[ERROR] Matrix inversion requires LAPACK");
    return false;
  }
#endif
  ;
...
#if !defined(HAVE_LAPACK)
  {
    ...
    printf("[ERROR] Eigenvalue computations requires LAPACK");
    return false;
  }
#endif
  ;
...
#if !defined(HAVE_LAPACK)
  {
    ...
    printf("[ERROR] Singular value decomposition requires LAPACK");
    return false;
  }
#endif
  ;
