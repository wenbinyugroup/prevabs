...
createElementMSH2(GModel *m, int num, int typeMSH, int physical, int reg,
  ...
  if (part)
    m->getMeshPartitions().insert(part);
  return e;
}

#if (FAST_ELEMENTS == 0)

static bool getElementsByNum(int elemNum[],
...
static MElement *
getParent(int parentNum, int type,
          std::map<int, std::vector<MElement *>> elements[10]) {
  ...
  case MSH_POLYG_:
  case MSH_POLYG_B:
    if (getElementsByNum(&parentNum, elements[2], true, &parent))
      return parent;
    if (getElementsByNum(&parentNum, elements[3], true, &parent))
      return parent;
    if (getElementsByNum(&parentNum, elements[8], true, &parent))
      return parent;
    return parent;
  case MSH_POLYH_:
    if (getElementsByNum(&parentNum, elements[4], true, &parent))
      return parent;
    if (getElementsByNum(&parentNum, elements[5], true, &parent))
      return parent;
    if (getElementsByNum(&parentNum, elements[6], true, &parent))
      return parent;
    if (getElementsByNum(&parentNum, elements[7], true, &parent))
      return parent;
    if (getElementsByNum(&parentNum, elements[9], true, &parent))
      return parent;
    return parent;
  default:
    return parent;
    ...
    if (!strncmp(&str[1], "MeshFormat", 10)) {
      ...
      for (int i = 0; i < numVertices; i++) {
        ...
        if (!parametric) {
          ...
          if (!binary) {
            ...
            if (swap)
              SwapBytes((char *)&iClasDim, sizeof(int), 1);
            if (fread(&iClasTag, sizeof(int), 1, fp) != 1) {
              fclose(fp);
              return 0;
            }
            if (swap)
              SwapBytes((char *)&iClasTag, sizeof(int), 1);
          }
          if (iClasDim == 0) {
            ...
            if (!binary) {
              ...
              if (swap)
                SwapBytes((char *)uv, sizeof(double), 1);
            }
            newVertex =
                new MEdgeVertex(xyz[0], xyz[1], xyz[2], ge, uv[0], -1.0, num);
          } else if (iClasDim == 2) {
            ...
            if (!binary) {
              ...
              if (swap)
                SwapBytes((char *)uv, sizeof(double), 2);
            }
            newVertex =
      ...
      if (!binary) {
        for (int i = 0; i < numElements; i++) {
          ...
          if (version <= 1.0) {
            if (fscanf(fp, "%d %d %d %d %d", &num, &type, &physical,
                       &elementary, &numVertices) != 5) {
              fclose(fp);
              return 0;
            }
          ...
          if (parent != 0) {
            ...
            if (ite == elems.end())
              Msg::Error("Parent element (ascii) %d not found for element %d "
                         "of type %d",
                         parent, num, type);
            else {
              p = ite->second;
            ...
            if (itp == parents.end()) {
              ...
              if (p)
                ...
                Msg::Error("Parent element %d not found for element %d dedju",
                           parent, num);
            } else
              p = itp->second;
            std::set<MElement *>::iterator itpo = parentsOwned.find(p);
          ...
          if (CTX::instance()->mesh.ignorePartBound && elementary < 0)
            continue;
        ...
        while (numElementsPartial < numElements) {
          ...
          for (int i = 0; i < numElms; i++) {
            ...
            if (parent) {
              ...
              if (ite == elems.end())
                Msg::Error(
                    "Parent (binary) element %d not found for element %d",
                    parent, num);
              else {
                p = ite->second;
                parents[parent] = p;
              ...
              if (itp == parents.end()) {
                ...
                if (p)
                  ...
                  Msg::Error("Parent (binary) element %d not found", parent);
              } else
                p = itp->second;
              std::set<MElement *>::iterator itpo = parentsOwned.find(p);
              if (itpo == parentsOwned.end()) {
...
static void writeElementMSH(FILE *fp, GModel *model, T *ele, bool saveAll,
  ...
  if (saveAll)
    ...
    if (parentNum)
      parentNum = parentNum - physicals.size() + 1;
...
template <class T>
static void writeElementSC(FILE *fp, GModel *model, T *ele, bool saveAll,
                           double version, bool binary, int &num,
                           int elementary, std::vector<int> &physicals,
                           std::vector<std::vector<double>> theta1,
                           std::vector<std::vector<double>> &ele_theta1,
                           int eletype, int vabs, std::vector<int> &groups,
                           int parentNum = 0, int dom1Num = 0,
                           int dom2Num = 0) {
  // current version only has triangle elements
  int n = ele->getNumVerticesForMSH();
  if (eletype == 2 && n == 6) {
    for (unsigned int j = 0; j < physicals.size(); j++) {
      if (vabs == 1) {
        // vabs
        fprintf(fp, "%8d", ++num);
        groups.push_back(abs(physicals[j]));
      } else {
        // swiftcomp
        fprintf(fp, "%8d%8d", ++num, abs(physicals[j]));
      }

      for (int i = 0; i < n; i++) {
        // for triangle element, the 4th node id is 0
        if (i == 2)
          fprintf(fp, "%8d%8d", ele->getVertex(i)->getIndex(), 0);
        else
          fprintf(fp, "%8d", ele->getVertex(i)->getIndex());
      }
      if (vabs == 1)
        fprintf(fp, "%8d%8d\n", 0, 0);
      else
    ...
    ele_theta1.push_back(theta1[elementary - 1]);
  } else if (eletype == 1 && n == 3) {
    for (unsigned int j = 0; j < physicals.size(); j++) {
      if (vabs == 1) {
        fprintf(fp, "%8d", ++num);
        groups.push_back(abs(physicals[j]));
      ...
      if (vabs == 1)
        ...
        fprintf(fp, "%8d%8d%8d%8d%8d%8d%8d%8d%8d%8d%8d%8d%8d%8d%8d%8d%8d\n", 0,
...
template <class T>
static void writeElementsSC(FILE *fp, GModel *model, std::vector<T *> &ele,
                            bool saveAll, int saveSinglePartition,
                            double version, bool binary, int &num,
                            int elementary, std::vector<int> &physicals,
                            std::vector<std::vector<double>> theta1,
  ...
  for (unsigned int i = 0; i < ele.size(); i++) {
    ...
    if (parent)
      parentNum = model->getMeshElementIndex(parent);
    writeElementSC(fp, model, ele[i], saveAll, version, binary, num, elementary,
                   physicals, theta1, ele_theta1, eletype, vabs, groups,
                   parentNum);
  }
}

static int getNumElementsMSH(GEntity *ge, bool saveAll,
                             int saveSinglePartition) {
  int n = 0, p = saveAll ? 1 : ge->physicals.size();

  if (saveSinglePartition < 0 && ge->tag() < 0)
    p = 1; // partition boundary
...
int GModel::_writeMSH2(const std::string &name, double version, bool binary,
  ...
  if (!CTX::instance()->mesh.saveTri) {
    for (viter it = firstVertex(); it != lastVertex(); ++it)
      for (unsigned int i = 0; i < (*it)->points.size(); i++)
        if ((*it)->points[i]->ownsParent())
          writeElementMSH(fp, this, (*it)->points[i]->getParent(), saveAll,
                          version, binary, num, (*it)->tag(), (*it)->physicals);
    for (eiter it = firstEdge(); it != lastEdge(); ++it)
      for (unsigned int i = 0; i < (*it)->lines.size(); i++)
        if ((*it)->lines[i]->ownsParent())
          writeElementMSH(fp, this, (*it)->lines[i]->getParent(), saveAll,
                          version, binary, num, (*it)->tag(), (*it)->physicals);
    for (fiter it = firstFace(); it != lastFace(); ++it)
      for (unsigned int i = 0; i < (*it)->triangles.size(); i++)
        if ((*it)->triangles[i]->ownsParent())
          writeElementMSH(fp, this, (*it)->triangles[i]->getParent(), saveAll,
                          version, binary, num, (*it)->tag(), (*it)->physicals);
    for (riter it = firstRegion(); it != lastRegion(); ++it)
      for (unsigned int i = 0; i < (*it)->tetrahedra.size(); i++)
        if ((*it)->tetrahedra[i]->ownsParent())
          writeElementMSH(fp, this, (*it)->tetrahedra[i]->getParent(), saveAll,
                          version, binary, num, (*it)->tag(), (*it)->physicals);
    for (fiter it = firstFace(); it != lastFace(); ++it)
      for (unsigned int i = 0; i < (*it)->polygons.size(); i++)
        if ((*it)->polygons[i]->ownsParent())
          writeElementMSH(fp, this, (*it)->polygons[i]->getParent(), saveAll,
                          version, binary, num, (*it)->tag(), (*it)->physicals);
    for (riter it = firstRegion(); it != lastRegion(); ++it)
      for (unsigned int i = 0; i < (*it)->polyhedra.size(); i++)
        if ((*it)->polyhedra[i]->ownsParent())
          writeElementMSH(fp, this, (*it)->polyhedra[i]->getParent(), saveAll,
                          version, binary, num, (*it)->tag(), (*it)->physicals);
  }
  // points
  for (viter it = firstVertex(); it != lastVertex(); ++it)
