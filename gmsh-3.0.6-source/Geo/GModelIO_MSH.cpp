...
static void readMSHEntities(FILE *fp, GModel *gm) {
  int nv, ne, nf, nr;
  int tag;
  if (fscanf(fp, "%d %d %d %d", &nv, &ne, &nf, &nr) != 4)
    return;
  for (int i = 0; i < nv; i++) {
    if (fscanf(fp, "%d", &tag) != 1)
      return;
    ...
    if (!readMSHPhysicals(fp, gv))
      return;
  }
  for (int i = 0; i < ne; i++) {
    int n;
    if (fscanf(fp, "%d %d", &tag, &n) != 2)
      return;
    ...
    if (!ge) {
      ...
      for (int j = 0; j < n; j++) {
        ...
        if (!v)
          Msg::Error("Unknown GVertex %d", tagv);
        if (j == 0)
          v1 = v;
        if (j == 1)
          v2 = v;
...
void readMSHPeriodicNodes(FILE *fp, GModel *gm) {
  ...
  for (int i = 0; i < count; i++) {
    ...
    if (fscanf(fp, "%d %d %d", &dim, &slave, &master) != 3)
      continue;
    ...
    // $Nodes section
      ...
      for (int i = 0; i < numVertices; i++) {
        ...
        if (!binary) {
          ...
          if (swap)
            SwapBytes((char *)&entity, sizeof(int), 1);
        }
        if (!entity) {
          ...
          if (!binary) {
            ...
            if (swap)
              SwapBytes((char *)&dim, sizeof(int), 1);
          }
          switch (dim) {
          case 0: {
          ...
          case 1: {
            ...
            if (!binary) {
              ...
              if (swap)
                SwapBytes((char *)&u, sizeof(double), 1);
            }
          ...
          case 3: {
            ...
            if (!binary) {
              ...
              if (swap)
                SwapBytes((char *)uvw, sizeof(double), 3);
            }
    ...
    // $Elements section
      ...
      for (int i = 0; i < numElements; i++) {
        ...
        if (!binary) {
          ...
          if (swap)
            SwapBytes((char *)&numData, sizeof(int), 1);
        }
        std::vector<int> data;
        if (numData > 0) {
          data.resize(numData);
          if (!binary) {
            ...
            if (swap)
              SwapBytes((char *)&data[0], sizeof(int), numData);
          }
        }
        MElementFactory f;
        ...
        case TYPE_PNT:
          elements[0][entity].push_back(element);
          break;
        case TYPE_LIN:
          elements[1][entity].push_back(element);
          break;
        case TYPE_TRI:
          elements[2][entity].push_back(element);
          break;
        case TYPE_QUA:
          elements[3][entity].push_back(element);
          break;
        case TYPE_TET:
          elements[4][entity].push_back(element);
          break;
        case TYPE_HEX:
          elements[5][entity].push_back(element);
          break;
        case TYPE_PRI:
          elements[6][entity].push_back(element);
          break;
        case TYPE_PYR:
          elements[7][entity].push_back(element);
          break;
        case TYPE_TRIH:
          elements[8][entity].push_back(element);
          break;
        case TYPE_POLYG:
          elements[9][entity].push_back(element);
          break;
        case TYPE_POLYH:
          elements[10][entity].push_back(element);
          break;
        }
...
void writeMSHEntities(FILE *fp, GModel *gm) {
  ...
  for (GModel::fiter it = gm->firstFace(); it != gm->lastFace(); ++it) {
    ...
    for (std::list<GEdge *>::iterator ite = edges.begin(); ite != edges.end();
         ite++)
  ...
  for (GModel::riter it = gm->firstRegion(); it != gm->lastRegion(); ++it) {
    ...
    std::vector<int> tags, signs;
    for (std::list<GFace *>::iterator itf = faces.begin(); itf != faces.end();
         itf++)
...
void writeMSHPeriodicNodes(FILE *fp, std::vector<GEntity *> &entities,
  ...
  for (unsigned int i = 0; i < entities.size(); i++)
    if (entities[i]->meshMaster() != entities[i])
      count++;
  if (!count)
    return;
  fprintf(fp, "$Periodic\n");
